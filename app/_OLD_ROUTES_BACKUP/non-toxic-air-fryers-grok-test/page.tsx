import productData from "@/data/products/non-toxic-air-fryers-grok-test.json";
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <nav className="mb-6">
          <ol className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <li>/</li>
            <li className="text-slate-900">Non-Toxic Air Fryers (Grok Test)</li>
          </ol>
        </nav>

        <div className="mb-10">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 mb-4">
            Best {productData.searchIntent} (2025) - GROK TEST
          </h1>
          <div className="flex flex-wrap items-center gap-2 mb-5">
            <time className="bg-slate-100 text-slate-600 text-sm font-medium px-3 py-1 rounded-lg">
              {new Date(verdict.verifiedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </time>
          ...
          <div className="bg-emerald-50 text-emerald-800 px-5 py-4 rounded-lg border border-emerald-200">
            <p className="text-sm font-medium leading-relaxed">
              <strong>The Winner:</strong> {verdict.summary} <span className="text-emerald-600">Verified {new Date(verdict.verifiedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
            </p>
          </div>
        </div>
                ...
                <div className="flex flex-col p-8 lg:p-12 lg:pr-8 items-center justify-center">
                  ...
                  <div className="flex items-center gap-3 mb-8 justify-center">
                    <div className="flex gap-0.5">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className={`w-5 h-5 ${i < Math.floor(winner.rating) ? 'fill-amber-400' : 'fill-slate-300'}`} viewBox="0 0 20 20">
                  ...
                  <div className="hidden lg:grid grid-cols-2 gap-2 w-full max-w-md mx-auto mb-8">
                    {["Stainless Steel", "No Coating", "No Prop 65", "Top 5 Ranked"].map((badge, i) => (
